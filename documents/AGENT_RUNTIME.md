...
保留从 s01 到 s17 的实现说明，供阅读源码和扩展运行时使用。下文默认描述 **CLI 完整运行时**；
Web 面试使用显式工具白名单与隔离模式，不接入全局 hooks、记忆、cron 和团队状态。
Web 架构及实时协议见 [Web 前端说明](WEB_FRONTEND.md)。
...
- **主循环**：手动循环——发请求 → 检查 `tool_use` → 逐个执行 → 结果回传，直到模型不再需要工具。
  循环在工具执行前后和回合结束时触发 hooks，并集成压缩、记忆与通知机制。

## Hooks（挂在循环上）

权限检查、工具日志和退出判断通过 hooks 扩展（参考 learn-claude-code s04）：

| 事件 | 触发时机 | 返回值的作用 |
| ---- | ---- | ---- |
| `UserPromptSubmit` | 用户输入后、进入 LLM 前 | 当前 `ask()` 触发事件，但未使用回调返回值 |
...
当前注册的 hooks：`permission_hook`（权限三道闸门）、`log_hook`（打印 [HOOK] 日志）、`summary_hook`（退出前统计工具调用次数）、`_goal_stop_hook`（目标判断与自动续轮）。
...
内置技能：`code-review`（代码审查清单）、`deepseek-api`（接口注意事项）、`mock-interviewer`（面试流程）。
...
| 3. `micro_compact` | 仍超 150K 字符 | 更早的已读长结果替换成路径引用（保留最近 3 条） | 0 |
...
（当前主循环按顺序执行工具，但模型在同一次回复中无法引用尚未返回的 ID）。
